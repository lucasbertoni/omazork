...
const testVerbs = `{"schemaVersion":1,"fastVerbs":["look","pray"],"verbs":[
 {"verb":"take","synonyms":["get"],"class":"manipulation","minutes":1},
 {"verb":"dig","class":"mechanism","minutes":10}]}`

const testTable = `{"schemaVersion":1,"game":"zork1",
 "rooms":[{"id":"KITCHEN","obj":10,"name":"Kitchen"},{"id":"CELLAR","obj":11,"name":"Cellar"}],
 "edges":[{"from":"KITCHEN","to":"CELLAR","dir":"DOWN","kind":"plain","minutes":4,"class":"movement","source":"rule","note":"base 2 + down 1 + dark 1"}],
...
func TestLoadWithoutOverlay(t *testing.T) {
	...
	if !ok || row.Minutes != 4 || row.Source != SourceRule {
		t.Fatalf("edge = %+v, %v", row, ok)
	}
	if row, ok := l.Action("dig", "SAND"); !ok || row.Minutes != 12 {
		t.Fatalf("action = %+v, %v", row, ok)
	}
	if row, ok := l.VerbDefault("dig"); !ok || row.Minutes != 10 || row.Class != ClassMechanism {
...
func TestOverlayOverridesAddsAndForcesInstant(t *testing.T) {
	fsys := baseFS()
	fsys["data/actions/zork1.overlay.json"] = &fstest.MapFile{Data: []byte(`{"schemaVersion":1,"game":"zork1",
	 "edges":[{"from":"KITCHEN","to":"CELLAR","minutes":0,"reason":"the trapdoor drop is instant"}],
	 "actions":[{"verb":"take","object":"SAND","minutes":90,"class":"dramatic","reason":"a curated set piece"}],
	 "verbDefaults":{"dig":20}}`)}
	l := loadOK(t, fsys)

	row, ok := l.Edge("KITCHEN", "CELLAR")
	if !ok || row.Minutes != 0 || row.Source != SourceOverlay {
		t.Fatalf("overridden edge = %+v, %v", row, ok)
	}
	added, ok := l.Action("take", "SAND")
	if !ok || added.Minutes != 90 {
		t.Fatalf("added action = %+v, %v", added, ok)
	}
	if row, ok := l.VerbDefault("dig"); !ok || row.Minutes != 20 || row.Source != SourceOverlay {
...
func TestLoadRejectsSchemaMismatch(t *testing.T) {
	for _, file := range []string{"data/actions/zork1.json", "data/actions/verbs.json"} {
		fsys := baseFS()
		fsys[file] = &fstest.MapFile{Data: []byte(`{"schemaVersion":2,"game":"zork1"}`)}
		if _, err := LoadFS(fsys, "zork1"); err == nil {
			t.Fatalf("%s: schemaVersion 2 accepted", file)
		}
	}
}
...
func TestLoadRejectsInvalidData(t *testing.T) {
	fsys := baseFS()
	fsys["data/actions/zork1.overlay.json"] = &fstest.MapFile{Data: []byte(`{"schemaVersion":1,"game":"zork1",
	 "edges":[{"from":"KITCHEN","to":"ATTIC","minutes":3}]}`)}
	if _, err := LoadFS(fsys, "zork1"); err == nil {
