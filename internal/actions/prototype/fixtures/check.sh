...
# Runs the ticket-#22 capture harness and prints a compact report:
# turn/score/move totals, suspicious turns, and the last N outputs.
set -euo pipefail
GAME=$1 SEED=$2 SCRIPT=$3 TAIL=${4:-3}
ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
OUT=$(mktemp)
trap 'rm -f "$OUT"' EXIT
(cd "$ROOT" && go run ./internal/actions/prototype capture "$GAME" "$SEED" < "$SCRIPT") > "$OUT"
